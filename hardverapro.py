...
client = mqtt.Client(client_id=f"hardverapro-{uuid.uuid4()}")
valid_listing_html = []
...
def extract_price(div):
    price_container = div.select_one('[data-testid*="price"]')
    if price_container:
        text = price_container.get_text(" ", strip=True)
        match = re.search(r"([\d\s]*[,\.]?\d*)\s*(Ft|€|\$)", text)
        if match:
            number_clean = int(re.sub(r"[^\d]", "", match.group(1)))
            return str(number_clean)

    price_container = div.select_one(".uad-price")
    if price_container:
        text = price_container.get_text(" ", strip=True)
        match = re.search(r"([\d\s]*[,\.]?\d*)\s*(Ft|€|\$)", text)
        if match:
...
def on_connect(client, userdata, flags, rc):
    client.subscribe("router/hardverapro/raw")
    client.subscribe("kill/hardverapro")
    client.publish("status", "hardverapro")
